...
public class TestCuenta extends TestCase {
	...
	public void testtrasferenciaokey () {
		...
		Cliente laura = new Cliente("12346X", "Laura", "Carrasco");
		laura.insert();
		Cuenta cuentaLaura = new Cuenta(2);
		try {
			cuentaPepe.addTitular(pepe);
			cuentaPepe.insert();
			cuentaPepe.ingresar(1000);
			cuentaLaura.addTitular(laura);
			cuentaLaura.insert();
			cuentaPepe.transferir(2L, 500, "alquile");
			assertTrue(cuentaPepe.getSaldo()==495);
			assertTrue(cuentaLaura.getSaldo()==500);
